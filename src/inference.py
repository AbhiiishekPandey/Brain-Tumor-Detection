...
from src.explainability.explainable import GradCAMPP, load_model, preprocess_image, visualize_and_save

# CONFIG
MODEL_PATH = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'models', 'best_model.pth')
CLASS_LABELS = ['glioma', 'meningioma', 'pituitary', 'notumor'] # Matched with explainable.py
...
class InferenceEngine:
    ...
    def predict(self, image_path, output_dir):
        ...
        # Generate Grad-CAM++
        # We need hooks for backward pass, so we might need a small gradient-based call here
        # Generate heatmap
